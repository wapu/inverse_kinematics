...
class InverseKinematicsModel():
    ...
    def draw_isolines(self, samples, color, filter_width):
        ...
        plt.contourf(X, Y, hist.T, [q,np.max(hist)], colors='#96BF0D', alpha=0.2, linewidths=0.7, zorder=-10)

    def init_plot(self, y_target):
        return plt.figure(figsize=(8,8))

    def update_plot(self, x, y_target, exemplar=None, filter_width=10, arrows=False, target_label=False):
        plt.gcf().clear()
        x = np.array(x)[:4000, :]
        if exemplar is None:
        ...
        plt.scatter(x3[:,0], x3[:,1], s=3, edgecolor='#96BF0D', facecolor='white',
            alpha=0.2, linewidths=0.5, zorder=-5, rasterized=True)
        ...
        if arrows:
            ...
            plt.text(-0.11, -0.60, r'$x_1$', ha='center', va='center', fontsize=10)
            plt.text( 0.13, -0.38, r'$x_2$', ha='center', va='center', fontsize=10)
            plt.text( 0.60, -0.36, r'$x_3$', ha='center', va='center', fontsize=10)
            plt.text( 1.10, -0.32, r'$x_4$', ha='center', va='center', fontsize=10)
            plt.text( 2.1, 0.23, r'$\mathbf{y}$', ha='center', va='center', fontsize=10)
...
if __name__ == '__main__':
    ...
    # train_data = InverseKinematicsDataset(model, 10000, 'kin_data', suffix='train')
    # train_loader = train_data.get_dataloader(1000)

    plt.figure(figsize=(3,3))
    np.random.seed(0)
    model.update_plot(model.sample_prior(4000), arrows=True, y_target=0)
